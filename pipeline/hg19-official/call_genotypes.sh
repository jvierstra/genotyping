...
usage () {
  echo -e "Usage: $0 <input-dir> <output-dir>" >&2
  exit 2
}
...
FASTA_CHROM_FILE=$(readlink -f ../data.references/hg19.chrom_sizes.bed)
FASTA_FILE=$(readlink -f ../data.references/hg19.fa)
DBSNP=$(readlink -f ../data.references/dbSNP/All_20180423.fixed-chrom.vcf.gz)

input_dir=$1
output_dir=$2

# Make BAM file list
ls ${input_dir}/*bam > ${output_dir}/filelist.txt
...
bcftools concat -f \${TMPDIR}/mergelist.txt -Ob > \${TMPDIR}/merged.bcf.gz
tabix -p bcf \${TMPDIR}/merged.bcf.gz
bcftools annotate -Oz -a ${DBSNP} -c ID --threads 16 \${TMPDIR}/merged.bcf.gz \
